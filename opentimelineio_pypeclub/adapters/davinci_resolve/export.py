...
def create_otio_clip(track_item):
    ...
    if not _bmdr.project_fps:
        fps = mp_clip_property["FPS"]
    else:
        fps = _bmdr.project_fps
...
def create_otio_track(track_type, track_name):
    return otio.schema.Track(
        name=track_name,
        kind=_bmdr.track_types[track_type]
    )
...
def create_otio_timeline(resolve_project):
    ...
    _bmdr.project_fps = resolve_project.GetSetting("timelineFrameRate")
    timeline = resolve_project.GetCurrentTimeline()

    # convert timeline to otio
    otio_timeline = _create_otio_timeline(
        resolve_project, timeline, _bmdr.project_fps)

    # loop all defined track types
    for track_type in list(_bmdr.track_types.keys()):
